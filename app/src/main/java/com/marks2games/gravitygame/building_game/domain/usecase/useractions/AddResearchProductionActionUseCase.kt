...
class AddResearchProductionActionUseCase @Inject constructor()  {
    operator fun invoke(actions: List<Action>, planetId: Int?, value: Int): List<Action> {
        val action = planetId?.let { Action.SetProduction.ResearchProduction(value, it) }
        val newActions = actions.toMutableList()
        val isActionSet = actions.find {it.type == ActionEnum.RESEARCH_ACTION && it.planetId == planetId }
        action?.let {
            if(isActionSet == null){
                newActions.add(it)
            } else {
